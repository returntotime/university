...
class U_univer{
	...
	function u_univer_scripts_styles() {
		global $wp_styles;
	...
	function register_configuration(){
		global $u_univer_settings;
		$u_univer_settings = new Options_Page('u_univer_settings', array('option_file'=>dirname(__FILE__) . '/options.xml','menu_title'=>__('U-Project Settings','cactusthemes'),'menu_position'=>null), array('page_title'=>__('U-Project Setting Page','cactusthemes'),'submit_text'=>'Save'));
	}
	...
	function get_option($option_name, $op_id = ''){
		return $GLOBALS[$op_id != ''?$op_id:'u_univer_settings']->get($option_name);
	}
	
	function init(){
		// Variables
		$this->template_url			= apply_filters( 'u_univer_template_url', 'u-project/' );
		...
		add_action( 'wp_enqueue_scripts', array($this, 'u_univer_scripts_styles') );
	}	
	/**
	 * Get the plugin path.
	 *
	...
	function template_loader($template){
		...
		if(is_post_type_archive( 'u_univer' ) || is_page('project') || is_tax('u_univer_cat')){
			$file = 'project-listing.php';
			$find[] = $file;
			$find[] = $this->template_url . $file;
		}
		elseif(is_singular('u_univer')){
	...
	function template_redirect(){
		...
		if ( ! empty( $_GET['page_id'] ) && get_option( 'permalink_structure' ) == "" && $_GET['page_id'] ==  'project' ) {
			wp_safe_redirect( get_post_type_archive_link('u_univer') );
			exit;
		}
	}
	
	function register_taxonomies(){
		$this->register_u_univer();
	}
	
	/* Register u_univer post type and its custom taxonomies */
	function register_u_univer(){
		$labels = array(
		...
		register_post_type( 'u_univer', $args );
		
		/* Register project Categories */
		$u_univer_cat_labels = array(
			'name'=>__('U-Project Categories','cactusthemes'),
			'singular_name'=>__('U-Project Category','cactusthemes')
		);
		$u_univer_tag_labels = array(
			'name'=>'U-Project Tags',
			'singular_name'=>'U-Project Tags'
		);
		//register_taxonomy('u_univer_tags', 'u_univer', array('labels'=>$u_univer_tag_labels,'meta_box_cb'=>array($this,'u_univer_type_meta_box_cb')));
		register_taxonomy('u_univer_cat', 'u_univer', array('labels'=>$u_univer_cat_labels,'show_admin_column'=>true,'hierarchical'=>true,'rewrite'=>array('slug'=>$slug_cat_project),'meta_box_cb'=>array($this,'u_univer_categories_meta_box_cb')));
	}
		
	/* Register meta box for Store Type 
	 * Wordpress 3.8
	 */
	function u_univer_type_meta_box_cb($post, $box){
	 ...
	 * @param object $post
	 */
	function u_univer_categories_meta_box_cb( $post, $box ) {
...
if ( ! function_exists( 'u_univer_get_page_id' ) ) {
	 ...
	 * @return int
	 */
	 /* function u_univer_get_page_id( $page ) {
		  global $affiliatez;
