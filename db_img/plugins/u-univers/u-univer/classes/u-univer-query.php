...
class U_univer_Query{
	...
	function pre_get_posts($q){
		...
		if ( $q->is_home() && 'page' == get_option('show_on_front') && get_option('page_on_front') == u_univer_get_page_id( 'univer' ) ) {
			$_query = wp_parse_args( $q->query );
			if ( empty( $_query ) || ! array_diff( array_keys( $_query ), array( 'preview', 'page', 'paged', 'cpage', 'orderby' ) ) ) {
				$q->is_page = true;
				$q->is_home = false;
				$q->set( 'page_id', get_option('page_on_front') );
				$q->set( 'post_type', 'u_univer' );
			}
		}

		// Special check for shops with the product archive on front
		
		if ( $q->is_page() && 'page' == get_option( 'show_on_front' ) && $q->get('page_id') == u_univer_get_page_id( 'univer' ) ) {
			...
			$shop_page 	= get_post( u_univer_get_page_id( 'univer' ) );
			$q->is_page = true;

			$wp_post_types['u_univer']->ID 			= $shop_page->ID;
			$wp_post_types['u_univer']->post_title 	= $shop_page->post_title;
			$wp_post_types['u_univer']->post_name 	= $shop_page->post_name;
		    ...
		    if 	( ! $q->is_post_type_archive( 'u_univer' ) && ! $q->is_tax( get_object_taxonomies( 'u_univer' ) ) )
		   		return;
